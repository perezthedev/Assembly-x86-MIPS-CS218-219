;	Assignment #7 - Floats and Local Variables
;	Ryan Aponte
;	Section 1004
;	10/16/2020
;	Description:This program is designed to explore the use of floating points and 
;	local variables to calculate triangle hypotenuses and find the median hypotenuse
...
;	Local Variable Allocation - 75 * 4 = 300 bytes
...
;	C[i] = sqrt(A[i]^2 + B[i]^2)
	...
	mov rax, ARRAY_SIZE
	findHypotenusesLoop:
		movss xmm0, dword[rdi  + rbx * 4]
		mulss xmm0, xmm0
		cvtsi2ss xmm1, dword[rsi  + rbx * 4]
		mulss xmm1, xmm1
		addss xmm0, xmm1
		sqrtss xmm0, xmm0
;		Store in local array
		movss dword[rbp - ARRAY_SIZE * 4 + rbx * 4], xmm0
		inc rbx
		cmp rbx, rax
		jbe findHypotenusesLoop
	; loop done
...
;		Call combSort with address to list in stack
	mov r9, rbp
	sub r9, 300
	mov rdi, r9			; rdi = rbp - 300
	mov esi, eax		; array size
	call combSort
